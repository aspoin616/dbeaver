...
public abstract class TaskConfigurationWizard extends BaseWizard implements IWorkbenchWizard {
    ...
    public void createTaskSaveButtons(Composite parent, boolean horizontal, int hSpan) {
        if (getContainer().isSelectorMode()) {
            ...
            saveAsTaskButton = UIUtils.createDialogButton(panel, TaskUIMessages.task_config_wizard_group_task_button_save, new SelectionAdapter() {
                @Override
                public void widgetSelected(SelectionEvent e) {
                    saveTask();
                }
            });
            Link tasksLink = UIUtils.createLink(panel, TaskUIMessages.task_config_wizard_group_tasks_button_open, new SelectionAdapter() {
                @Override
                public void widgetSelected(SelectionEvent e) {
                    try {
                        UIUtils.getActiveWorkbenchWindow().getActivePage().showView(DatabaseTasksView.VIEW_ID);
                    } catch (PartInitException e1) {
                        DBWorkbench.getPlatformUI().showError(TaskUIMessages.task_config_wizard_group_task_title_show_error, "Error opening database tasks view", e1);
                    }
                }
            });
